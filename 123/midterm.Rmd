...
```{r ans51}
# twDates2
twDates <- str_remove(twDates,"民國")
twDates2 <- str_remove(twDates,"月")
twDates2
# or you can use str_reaplace
```
...
```{r}
twDates2
str_sub(twDates2,1,3) -> yearCharacter
...
DataSplitList #是個list

map(DataSplitList,~.[[2]]) -> monthList #取出每個元素的第二個值
monthList

unlist(monthList) -> monthVector #解開list
monthVector
...
```{r ans71}
# failRate
failRate <- sum(sampledTranscriptData$科目名稱=='科目204'& sampledTranscriptData$學期成績<60)/sum(sampledTranscriptData$科目名稱=='科目204')
failRate
```
...
sampledTranscriptData$系級<-str_replace_all(sampledTranscriptData$系級,'1|2|3|4','')
sampledTranscriptData$學系<- sampledTranscriptData$系級
sampledTranscriptData$學系
```
```{r}
